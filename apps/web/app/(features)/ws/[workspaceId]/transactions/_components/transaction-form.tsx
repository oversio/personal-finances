...
export interface TransactionFormRef {
  setValue: UseFormSetValue<CreateTransactionFormData>;
}
...
export const TransactionForm = forwardRef<TransactionFormRef, TransactionFormProps>(
  function TransactionForm(
    ...
    // Sync display when form amount changes externally (e.g., edit mode or invoice scan)
    useEffect(() => {
      if (currentAmount && currentAmount > 0) {
      ...
      <>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          <Textarea
            ...
            value={watch("notes") ?? ""}
            onValueChange={value => setValue("notes", value || undefined)}
